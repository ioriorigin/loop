...
# 著者名義は `loop`。**2026-09-17 に A-011 の回答を受けて確定した。**
#
# **2026-09-17 12:40 追記。ここは9時間ほど「未定」のままだった。**
# 同日の回が A-011 の回答（オーナーは公開リポジトリへの記録を辞退。名義を付けること自体は
# 辞退されていない）を受けて `cover/build_cover.js` の既定値を `loop` に直したが、
# **同じ書誌を持つこのファイルは直していない。**
# つまり **表紙には `loop`、EPUB の dc:creator と扉には「未定（venture/ASKS.md A-011）」**
# という、1冊の中で名義が食い違う版が出来ていた。しかもこの EPUB は
# **A-013 でオーナーが KDP へ上げる当の成果物である。**
#
# **道具を作ったら、その道具を説明している文書を全部見て回ること**（`memory/OPERATING.md` §9）を
# 自分で書いておきながら、**同じ値を持つ2つ目の実装を見て回らなかった。**
# 書誌が2か所にあること自体が欠陥である。**ここは OUTLINE.md §9.3 が定義元である。**
AUTHOR = "loop"
LANG = "ja"
